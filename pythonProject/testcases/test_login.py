...
class TestLogin(unittest.TestCase):
    ...
    read = ReadExcel(os.path.join(data_path,'testdata.xlsx'),'login')
    ...
    def test_login(self,case):
        ...
        #准备数据
        title = (case['title'])
        data = json.dumps(eval(case['testdata']))   #接口输入数据要求为json，所以需要用json.dumps转换一下
        ...
        # 断言
        try:
            self.assertEqual(excepted, res.text)   #接口返回的实际结果为json,需要转换成字典
